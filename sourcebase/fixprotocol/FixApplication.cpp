...
void CFixApplication::onCreate( const FIX::SessionID& sessionID)
{

}
void CFixApplication::onLogon( const Message&logon, const FIX::SessionID& sessionID )
{
	m_pCFixApplicationCB->HandleFixMessage(std::string(FIX::MsgType_Logon), logon, sessionID);
}

void CFixApplication::onLogout( const Message&logout, const FIX::SessionID& sessionID )
{

}

void CFixApplication::fromApp( const FIX::Message& message, const FIX::SessionID& sessionID )
throw( FIX::FieldNotFound, FIX::IncorrectDataFormat, FIX::IncorrectTagValue, FIX::UnsupportedMessageType )
{
	m_pCFixApplicationCB->HandleFixMessage(message.getHeader().getField(FIX::FIELD::MsgType), message, sessionID);
}

void CFixApplication::toApp( FIX::Message& message, const FIX::SessionID& sessionID )
throw( FIX::DoNotSend )
{
  try
  {
...
void CFixApplication::onLogonSLC( const SessionID& )
{
}
...
FIX::IntFIXMsgType CFixApplication::GetMgsIntType( std::string msgTypeValue )
{
	...
	if( msgTypeValue == "UAR" ) //Account Data Request
      intType = IntType_AccountDataRequest_UAR;
	else
	if( msgTypeValue == "UAD" ) //Account Data Report
      intType = IntType_AccountDataReport_UAD;
	else 
	if( msgTypeValue == "UAN" ) //Position Data Request
	  intType = IntType_RequestforPosition_UAN;
	else
	if( msgTypeValue == "UAF" ) //OrderMass Request
	  intType = IntType_OrderMassStatusRequest_UAF;
