...
:CServerBase(pName), m_ServiceName(pName->GetLocation())
{
	...
	{
		RAISE_RUNTIME_ERROR("set TTL failed");
	}
	...
	*p='\0';	
	...
	{
		RAISE_RUNTIME_ERROR("get hostip failed");
	}
	//绑定到多播组
#ifdef WIN32
	...
	{
		RAISE_RUNTIME_ERROR("set ip to imr_interface failed");
	}
	if(inet_pton(AF_INET,m_pMultiCastIP,&mcaddr.imr_multiaddr) <= 0)
	{
		RAISE_RUNTIME_ERROR("rong multicast IP address");
	}
#endif
	//指定多播从哪个网卡发送
	...
	{
		RAISE_RUNTIME_ERROR("setsockopt error,join IP_MULTICAST_IF failed"); 
	}
	...
	{
		RAISE_RUNTIME_ERROR("setsockopt error,join multicast failed"); 
	}
}
...
char *CMultiServer::GetMatchIP(char *pszMask,char *OutIP)
{
	...
	{
  		RAISE_RUNTIME_ERROR("gethostname error"); 
	}
	hp = gethostbyname(szName);
	if(NULL == hp)
	{
  		RAISE_RUNTIME_ERROR("gethostbyname error"); 
	}
