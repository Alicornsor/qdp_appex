...
//xuzh 20130607 合约号订阅
typedef CHashMap<DWORD,CInstrumentIDType,HashInt> CSubInstrumentMap;
...
struct	TFTDCHeader
{
	...
	//20130504 xuzh 增加合约号的报文头，用于区分是哪个合约
	DWORD	InstrumentIDCount;	/**< 合约号的整数值，不用解包就知道是哪个合约  4 二进制无符号整数。*/	
...
inline void TFTDCHeader::ChangeEndian()
{
	...
	CHANGE_ENDIAN(InstrumentIDCount);
}
...
public:
	...
	*/
	...
	inline void SetInstrumentIDCount(DWORD instrumentIDCount);

	inline DWORD GetInstrumentIDCount();
...
inline void CFTDCPackage::SetInstrumentIDCount(DWORD instrumentIDCount)
{
	m_FTDCHeader.InstrumentIDCount =instrumentIDCount;
}

inline DWORD CFTDCPackage::GetInstrumentIDCount()
{
	return m_FTDCHeader.InstrumentIDCount;
}
