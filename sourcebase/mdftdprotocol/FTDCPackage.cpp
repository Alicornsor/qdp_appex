...
void CFTDCPackage::OutputHeader(CLogger *pLogger)
{
	...
	pLogger->output(LL_DEBUG, "\tFieldCount[%d],FTDCContentLength[%d],RequestId[%d],InstrumentIDCount[%d]\n",
		pHeader->FieldCount, pHeader->FTDCContentLength, pHeader->RequestId,pHeader->InstrumentIDCount);	
}
