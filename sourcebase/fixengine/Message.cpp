                        ...
                        const DataDictionary& dataDictionary )
{
  ...
  while ( pos < string.size() )
  {
    ...
    (pDD->isField( field.getTag() ) && (!pGroup || pGroup->isSetField( field.getTag() )) ))
    {
      //if ( pGroup.get() )
	  if ( pGroup )
      {
        map.addGroupPtr( group, pGroup, false );
      }
	  ...
	  pGroup = new Group( field.getTag(), delim, pDD->getOrderedFields() ) ;
    }
