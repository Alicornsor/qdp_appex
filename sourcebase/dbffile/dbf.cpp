...
int CDBF::f4b_str(char *fieldname,const char *fvalue)
{
   TField *f_ptr;
   char *pb;
   f_ptr = f4findfield(fieldname);
...
//对于空数据文件（只有字段头），在包含DBF数据区域之后插入结束标记0x1A
int CDBF::writeEndFlag()
{
	lseek(handle,0,SEEK_END);
    BYTE endflag = 0x1A;
	
	if (write(handle,&endflag,1) != 1)
	{
		return(DBF_WRITEFAIL);
	}
	else 
