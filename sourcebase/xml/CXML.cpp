...
{
	// 2013-01-24 longqy 直接转换 CStringPtr 类型变量m_version 等为 (char *) 有点不符号规范。
...
void CXMLDoc::read(char *filename,int ignoreText,bool ignoreComment)
{
	...
	*/
	...
	input=mfopen(filename,"rt");
	if (input==NULL)
	{	// longqy 2014-02-28 增加输出错误信息，在打开配置文件出错的情况下，输出错误原因
		int errsv = errno;
...
CXMLElement *CXMLDoc::getPureNextElement(FILE *input)
{
	...
	{
		// delete name;
		// 2013-1-18 longqy  内存空间是由 malloc 分配，释放时用 free 比较规范
		free(name);
	}
	if (pString!=NULL)
	{
