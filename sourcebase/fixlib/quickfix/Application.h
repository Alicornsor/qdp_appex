...
public:
  ...
  virtual void fromAppSLC(const int& , const std::string& ) = 0;
  /// Notification of a session successfully logging on
  virtual void onLogonSLC( const SessionID& ) = 0;
...
//   { Locker l( m_mutex ); app().onLogon( sessionID ); }
  void onLogout(const Message& msg, const SessionID& sessionID )
  { Locker l( m_mutex ); app().onLogout(msg, sessionID ); }
