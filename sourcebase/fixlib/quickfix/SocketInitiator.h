...
{
/*** XLONG BEGIN ***/
class SocketInitiatorCB
{
public:
	SocketInitiatorCB(){}
	~SocketInitiatorCB(){}
	virtual void onConnect( const SessionID sessID ) = 0;
	virtual void onDisconnect( const SessionID sessID ) = 0;
private:
};
...
/*** XLONG BEGIN ***/
  SocketInitiator( Application&, MessageStoreFactory&,
	  const SessionSettings&, SocketInitiatorCB *pCB) throw( ConfigError );
  SocketInitiator( Application&, MessageStoreFactory&,
                   const SessionSettings&, LogFactory&,
				   SocketInitiatorCB *pCB) throw( ConfigError );
/*** XLONG END ***/
...
private:
  ...
  bool m_stop;
/*** XLONG BEGIN ***/
  SocketInitiatorCB *m_pSocketInitiatorCB;
/*** XLONG END ***/
};
/*! @} */
}
