...
public:
	...
	//20141222 huwp 由于要把不用结点放入内存节点池复用，放入池前需要恢复结点状态
	void ResetNode()
	{
		m_nLength = 0;
		m_pData = m_pBuffer;
	...
	void *PushBack(void *pData, int nDataLen)
	{
		...
		{			
			if (m_nLength == 0) 
			{
				m_pData = m_pBuffer;
				pTarget = m_pData;
				//20141222 huwp 修改之前胡国和发现的会导致内存越界的bug
...
void *CCacheList::PushBack(void *pData,int nDataLen)
{
	...
	//20141222 huwp 先检查内存节点池是否有空，否则分配一个新的节点
	CCacheNode *pNode = m_pCachePoolHead;
	if(NULL != pNode)
	{
...
int CCacheList::PopFront(int len)
{
	...
	if(m_pCacheHead->GetLength() <= 0)
	{
		...
		if(pNext != NULL)
		{
			...
			{
				//20141222 huwp 把要删除的节点放入内存节点池头部			
