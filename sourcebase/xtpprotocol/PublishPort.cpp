...
void CPublishPort::MoveTo(int nOffset, CFlowReader::SeekFlag nSeekFlag)
{
	...
	NET_STATUS_LOG3("Publish Port Move SubjectID=[%d] [%d] Move [%d]\n", m_nSubjectID, m_flowReader.GetId(),nOffset-m_flowReader.GetId());
...
int CPublishPort::Publish(DWORD nClockLapse,int DriveFlag)
{
	if (DriveFlag == 0)
		inter=nClockLapse;
