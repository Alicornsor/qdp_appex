	...
	SQLHANDLE Handle)
{
	...
	throw SAException(
		SA_DBMS_API_Error,
		NativeError, -1,
		_TSA("%") SA_FMT_STR, (const SAChar*)sMsg);
}
	...
	saConnectionHandler_t fHandler)
{
	...
	try
	{
		...
		SAString sOption = m_pSAConnection->Option(_TSA("SQL_DRIVER_PROMPT"));
		if(  ! sOption.IsEmpty() )
...
void IodbcCursor::SetFieldBuffer(
	...
	size_t nBufSize)
{
	if( getOptionValue(_TSA("ODBCUseSQLGetData"), false) )
	...
	int nBulkReadingBufPos) const
{
	...
	if( bLong )
	{
		...
		SQLRETURN rc = g_odbcAPI.SQLGetData(
			...
			TargetType, Buf,
#ifdef SQLAPI_WIN32
			bAddSpaceForNull ? sizeof(TSAChar) :
#endif
			0, &StrLen_or_IndPtr);
		...
		*vr.m_pbNull = StrLen_or_IndPtr == SQL_NULL_DATA;
	...
	*vr.m_pbNull = ((SQLLEN*)pInd)[nBulkReadingBufPos] == SQL_NULL_DATA;
	if(!vr.isNull())
