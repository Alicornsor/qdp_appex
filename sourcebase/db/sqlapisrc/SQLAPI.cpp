...
static void CheckTrial()
{
	...
	if(!bCheckedTrial)
	{
		...
		sa_printf(_TSA("%s") SA_FMT_STR _TSA("\n\n"), (const SAChar*)s);
...
const SAString &SAString::operator=(const std::string &stringSrc)
{
	*this = stringSrc.c_str();
	return *this;
}

const SAString &SAString::operator=(const std::wstring &stringSrc)
{
	*this = stringSrc.c_str();
	return *this;
}
...
#else
	m_tm = *localtime(&tmbValue.time);	// Convert to local time
#endif
...
SADateTime::SADateTime(const struct timeval &tmvValue)
{
	...
	m_tm = *localtime(&lt);	// Convert to local time
#endif
...
SADateTime::SADateTime(double dt)
{
	if( ! TmFromDate(dt, m_tm, m_nFraction) )
		...
		throw SAException(SA_Library_Error, -1, -1, IDS_DATETIME_WRONG_ERROR);
#else
		Init_Tm();
#endif
}
...
void SADateTime::GetTimeValue(SYSTEMTIME &st)
{
	...
	st.wMilliseconds = (WORD)(m_nFraction != 0 ? (m_nFraction / 1000):0);
}
#endif
	...
	size_t nEnd)	// param end position in SQL statemen
{
	...
	if( ! pRef )	// create param
	{
		...
		if( sa_isdigit(((const SAChar*)sName)[0]) )
		{
			...
			{
				m_ppParamsID = (SAParam**)realloc(m_ppParamsID, nID*sizeof(SAParam*));
...
void SACommand::setBatchExceptionPreHandler(PreHandleException_t fnHandler)
{
	ISACursor *pISACursor = m_pConnection? m_pConnection->GetISACursor(this) : NULL;
	if(!pISACursor)
		throw SAException(SA_Library_Error, -1, -1, IDS_CLIENT_NOT_SET);

	pISACursor->setBatchExceptionPreHandler(fnHandler);
}
...
{
	if(!sVal || !*sVal)
		throw SAException(SA_Library_Error, -1, -1, IDS_CONVERTION_FROM_STRING_TO_NUMERIC_ERROR, (const SAChar*)sVal);
...
{
	SetInterval(nDays, nHours, nMins, nSecs);
}
...
{
	SAString s;
	s.Format(_TSA("%") SA_FMT_STR _TSA("%02u:%02u:%02u"), m_interval < 0 ? _TSA("-"):_TSA(""),
		labs(GetHours() + GetDays() * 24), labs(GetMinutes()), labs(GetSeconds()));
	return s;
}

void SAInterval::SetInterval(long nDays, int nHours, int nMins, int nSecs)
{
