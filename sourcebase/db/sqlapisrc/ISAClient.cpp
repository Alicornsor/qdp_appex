...
size_t ISACursor::InputBufferSize(const SAParam &Param) const
{
	...
	{
	case SA_dtUnknown:
		throw SAException(SA_Library_Error, -1, -1, IDS_UNKNOWN_PARAMETER_TYPE,
...
void ISACursor::AllocBindBuffer(size_t nIndSize, size_t nSizeSize)
{
	...
	size_t* nCurParamSize = (size_t *) calloc(cParamCount, sizeof(size_t));
	size_t* nDataSize = (size_t *) calloc(cParamCount, sizeof(size_t));
	...
	m_pParamBuffer = realloc(m_pParamBuffer, nAllocatedSize);
								...
								size_t nAddlSize, void **ppAddl)
{
	...
	size_t* nCurParamSize =
		(size_t *) calloc(nPlaceHolderCount, sizeof(size_t));
	size_t* nDataSize = (size_t *) calloc(nPlaceHolderCount, sizeof(size_t));
								  ...
								  int nBulkReadingBufSize)
{
	...
	size_t* nCurFieldSize = (size_t *) calloc(cFieldCount, sizeof(size_t));
	size_t* nDataSize = (size_t *) calloc(cFieldCount, sizeof(size_t));
	...
	m_pSelectBuffer = ::realloc(m_pSelectBuffer, nAllocatedSize);
...
void ISACursor::ConvertOutputParams()
{
	...
	for( int i = 0; i < m_pCommand->ParamCount(); ++i )
	{
		...
		if( eParamType == SA_dtUnknown )
			throw SAException(SA_Library_Error, -1, -1,
											 ...
											 size_t nWantedSize, size_t &nDataSize, SAPieceType_t &eDataPieceType)
{
	...
	if( nNewReminderBytesCount )
	{
		...
		{
			m_chReminderBytes = (char*) ::realloc(m_chReminderBytes,
				m_MB_CUR_MAX);
										...
										size_t nWantedSize, size_t &nDataSize, SAPieceType_t &eDataPieceType)
{
	...
	if( nNewReminderBytesCount )
	{
		...
		{
			m_chReminderBytes = (unsigned char*) ::realloc(m_chReminderBytes, 2
				* sizeof(UTF16));
			while( m_ReminderBytesCount < nNewReminderBytesCount )
			{
				m_chReminderBytes[m_ReminderBytesCount]
...
void ISACursor::setBatchExceptionPreHandler(PreHandleException_t fnHandler)
{
...
bool ISACursor::PreHandleException(SAException &x)
{
	if( this->m_fnPreHandleException )
		return this->m_fnPreHandleException(x);
